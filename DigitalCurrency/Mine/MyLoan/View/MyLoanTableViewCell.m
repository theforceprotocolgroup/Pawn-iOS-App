...
/*!  */
@property (nonatomic, strong) MyLoanListModel* model;
...
- (void)viewModel:(RACTuple *)tuple action:(nullable KKActionHandle)action {
    ...
    NSString * title =[NSString stringWithFormat:@"出借金额%@%@",_model.investedCount,_model.investedTokenType];
    NSMutableAttributedString * att = [NSMutableAttributedString string:title];
    [att addAttribute:NSFontAttributeName value:KKCNBFont(15) range:NSMakeRange(0, 4)];
    [att addAttribute:NSFontAttributeName value:KKThirdFont(15) range:NSMakeRange(4, title.length - 4)];
    self.titleLabel.attributedText = att;
    self.subTitleLabel.text = [NSString stringWithFormat:@"%@%@%@",[_model subTitleText],_model.interestCount,_model.investedTokenType];
    self.statusLabel.text = [_model statusText];
    self.statusLabel.textColor = [_model statusColer];
    self.contentLabel.text = [NSString stringWithFormat:@"回款日：%@",_model.expectedRepaymentDate];
